...
| `Ctrl/⌘ + F` | 搜索已添加的站点 |
| `/` | 聚焦顶部搜索框（光标不在输入框里时） |
| `Esc` | 收起搜索联想条（也支持点击页面其它地方收起） |

顺手把「站点库」按挑选按钮从新增弹窗里删了（`SiteLibrary.vue` 已移除）——现在链接框直接支持
关键词模糊搜索（见下一节），不需要再开二级弹窗；「内置导航」按分类浏览还在（设置抽屉里）。
设置抽屉的顺序也调过：常用的（快捷操作 / 背景 / 布局 / 视图 / 字体 / 搜索 / 壁纸 / 排序 / 备份）
在前，**账号与 AI 配置挪到最后**。

**AI 自动分组怎么用？**
设置抽屉**最底部** → 「AI 自动分组」：填 **OpenAI 兼容接口**的 `接口地址`（含 `/v1`，默认
`https://api.deepseek.com/v1`）、`模型名`（默认 `deepseek-chat`）、`API Key`，保存后：

- **新增站点时自动识别分组**（不用点任何按钮）：抓完标题/图标后自动调用，结果**直接填进分组输入框**，
  用户可以改；自己手动改过分组就不再被覆盖；
- 「用 AI 给全部站点分组」按钮可一键给存量站点重排分组。

⚠️ **分组只是一层额外展示**：默认排布（平铺 / 手机翻页）完全不受分组影响，想看分组要手动切到
「视图 → 按分组」。2026-09-18 之前是"只要有站点带分组，首页就自动变成分组视图"，现在改成三选一：
**默认 / 字母索引 / 按分组**（设置抽屉的「视图」段，选择随偏好同步）。

取舍说明：

- **接口地址 / 模型：跟账号同步**（`user_data.key = 'ai'`，走 `/api/user/prefs` 的 LWW 合并）。
- **API Key：只写不读**（2026-09-18 定稿）。它单独存 `user_data.key = 'ai_key'`：
  - `PUT /api/settings/ai-key` 是唯一写入口（保存/清除），**任何接口都不回传**；
    `/api/user/prefs` 读到 `'ai'` 时会把 `apiKey` 剥掉、只补一个 `hasKey: true`（老数据自动搬到 `ai_key`）；
  - **浏览器 localStorage 里也不再保存 Key**（`useAi.js` 的 load/normalize 都会删掉 `apiKey`），
    所以浏览器控制台、扩展、XSS 都读不到；
  - `/api/ai/group` 请求里不带 Key，Worker 用当前会话的用户 id 去库里取
    （`functions/api/ai/group.js`），换设备登录后直接可用。
  - 仍存在的暴露面只剩**数据库本身**（明文，需要服务器/库的访问权限才能看到）。
    要再进一步就得上加密（密钥放 Worker 机密里），目前没做。
- 之所以不浏览器直连：国内直连 `api.deepseek.com` / `api.openai.com` 常超时或 CORS 被拦，
  Worker 在境外边缘，通道稳定（2026-09-18 实测：Worker 转发能正常收到大模型的 401 鉴权错误，
  说明链路通）。
- 端点是"转发器"，所以做了基本防护：必须登录、`apiBase` 必须是 **https 公网地址**
  （`isPrivateHost` 挡掉内网/本机，避免被当 SSRF 跳板）、单次最多 80 个站点（前端自动分批）。
- 分组会**覆盖现有分组名**；已有分组会作为候选传给模型，让它优先复用。
